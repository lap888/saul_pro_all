 ...
 * @Last Modified time: 2023-07-27 07:47:49
 */
...
client.exchangeInfo().then(async response => {
    ...
    client.listenKeyPost().then(response => {
        ...
        let sql = `select * from xq_coin where uId=${myUid} order by utime desc`
                        ...
                        coinInfo.forEach(vInfo => {
                            ...
                            if (symbol == curSymbol) {
                                ...
                                if (kTimeDate[symbol] == undefined || Date.now() - kTimeDate[symbol] > 0) {
                                    ...
                                    client.records(symbol, '30m', 1000).then(k => {
                                        ma90m[symbol] = TA.EMA(k, 90)
                                        let kp = k[k.length - 2]
                                        let kp2 = k[k.length - 3]
                                        let ma90mLast = _N(ma90m[symbol][ma90m[symbol].length - 2], 8)
                                        ...
                                        cci[symbol] = TA.CCI(k, 20)
                                        let c1 = cci[symbol][cci[symbol].length - 1]
                                        let c2 = cci[symbol][cci[symbol].length - 2]
                                        cci1[symbol] = (c1 > -100 && c1 > c2 && kp.Open > ma90mLast) || (c1 > 100 && c1 > c2 && kp.Open > ma90mLast) ? 'L' : (c1 < -100 && c1 < c2 && kp.Open < ma90mLast) || (c1 < 100 && c1 < c2 && kp.Open < ma90mLast) ? 'S' : ''
                                ...
                                if (longPos.length > 0 && longPos[0].pa != 0 && (Date.now() - OpenForderDate[symbol] > nextOrderWait || OpenForderDate[symbol] == undefined)) {
                                    ...
                                    if (longPos[0].pp < vInfo.totalAmount) {
                                        ...
                                        if (vInfo.lRightAddCount < vInfo.rightAddMax) {
                                            ...
                                            let isdol = symbolHyPrice[symbol] > vInfo.lPrice * (1 + 0.001) && longPos[0].pp < 10 * vInfo.rightPpAmount
                                            // console.log('vInfo.rightPpAmount / vInfo.level=',vInfo.rightPpAmount / vInfo.level)
                                    ...
                                    //回调50%止盈一半
                                    if (longPos[0].pa > 0 && longPos[0].up > vInfo.rightPpAmount && longPos[0].up < lIncomeMax[symbol] / 2 && (Date.now() - OpenOrderDate0[symbol] > 2 * 1000 || OpenOrderDate0[symbol] == undefined)) {
                                        OpenOrderDate0[symbol] = Date.now()
                                        pLog(`${symbol} 多单回调止盈 记录价格:${price} 下单数量:${longPos[0].pa / 2} 收益:${longPos[0].up / 2}`)
                                        updatePosAndSymbol(symbol, 'L', longPos[0].pa / 2, price, longPos[0].up / 2)
                                        let sql2 = `update xq_coin set lPrice=?,lIncome=0,utime=now() where id=?`
                                        doDb(sql2, [`${price}`, `${vInfo.id}`]).then(res2 => {
                                            console.log(symbol, '多单，回调止盈一半，更新数据库, ok')
                                        }).catch(e => {
                                            console.error(symbol, '多单，回调止盈一半，更新数据库', e)
                                        })
                                    ...
                                    //未持仓 开首单
                                    if ((OpenForderDate[symbol] == undefined || Date.now() - OpenForderDate[symbol] > 1000 * 30 * 60) && (longPos.length <= 0 || (longPos.length > 0 && longPos[0].pa == 0))) {
                                        //多单开仓 
                                        if (sarm1[symbol] == 'L' && cci1[symbol] == 'L') {
                                            OpenForderDate[symbol] = Date.now()
                                            let amount = _N(vInfo.fristAmount / symbolHyPrice[symbol], quantityPrecision[symbol])
                                            pLog(`${symbol} 多单开仓 下单记录价格:${price} 下单数量:${amount}`)
                                ...
                                if (shortPos.length > 0 && shortPos[0].pa != 0 && (Date.now() - OpenForderDate[symbol] > nextOrderWait || OpenForderDate[symbol] == undefined)) {
                                    ...
                                    if (shortPos[0].pp < vInfo.totalAmount) {
                                        ...
                                        if (vInfo.sRightAddCount < vInfo.rightAddMax) {
                                            ...
                                            let isdos = symbolHyPrice[symbol] < vInfo.sPrice * (1 - 0.001) && shortPos[0].pp < 10 * vInfo.rightPpAmount
                                    ...
                                    //回调50%止盈一半
                                    if (shortPos[0].pa > 0 && shortPos[0].up > vInfo.rightPpAmount && shortPos[0].up < sIncomeMax[symbol] / 2 && (Date.now() - OpenOrderDate0[symbol] > 2 * 1000 || OpenOrderDate1[symbol] == undefined)) {
                                        OpenOrderDate1[symbol] = Date.now()
                                        pLog(`${symbol} 空单回调止盈一半 记录价格:${price} 下单数量:${shortPos[0].pa / 2} 收益:${shortPos[0].up / 2}`)
                                        updatePosAndSymbol(symbol, 'S', shortPos[0].pa / 2, price, shortPos[0].up / 2)
                                        //更新数据库
                                    ...
                                    //未持仓 开首单 空
                                    if ((OpenForderDate[symbol] == undefined || Date.now() - OpenForderDate[symbol] > 60 * 30 * 1000) && (shortPos.length <= 0 || (shortPos.length > 0 && shortPos[0].pa == 0))) {
                                        if (sarm1[symbol] == 'S' && cci1[symbol] == 'S') {
                                            OpenForderDate[symbol] = Date.now()
                                            let amount = _N(vInfo.fristAmount / symbolHyPrice[symbol], quantityPrecision[symbol])
