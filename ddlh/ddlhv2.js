 ...
 * @Last Modified time: 2023-04-14 14:15:47
 */
    ...
    symbolDatas.forEach((v, i) => {
        if (v.symbol == symbol) {
            if (action == 'LONG') {
                ...
                if (isJs) {
                    symbolDatas[i].unLockCount = symbolDatas[i].unLockCount + 0.38
                }
                if (lockStatus) {
    ...
    doDb(sql, []).then(dbCoin => {
        ...
        client.combinedStreams(SymbolDataArrys, {
            ...
            message: (msg) => {
                ...
                if (UpdatePosDate == null || Date.now() - UpdatePosDate > 5000) {
                    ...
                    symbolDatas.forEach(info => {
                        ...
                        if ((shortPos.length <= 0 || shortPos[0]?.pa == 0) && (longPos.length <= 0 || longPos[0]?.pa == 0)) {
                            //空仓了 更新状态
                            let sql2 = 'update `ddlh_coin` set unLockCount=1,unLockIncome=0,lockStatus=0,utime=now() where id=?'
                ...
                //=====监控解锁锁仓的币种=====
                if (DodoDate1[symbol] == undefined || Date.now() - DodoDate1[symbol] > 0.3 * 1000) {
                        ...
                        coinInfoUnLock.forEach(vInfoUnlock => {
                            ...
                            if (lockSymbol == symbol) {
                                ...
                                if (longPos.length > 0) {
                                    ...
                                    if (lIncome > mIncome && lIncome < lIncomeMax[symbol] * 0.91) {
                                        //解锁多单
                                        ...
                                        //更新数据库
                                        let sql2 = 'update `ddlh_coin` set lPrice=?,unLockCount=unLockCount+0.38,unLockIncome=unLockIncome+?,totalIncome=totalIncome+?,lockStatus=0,utime=now() where id=?'
                                        doDb(sql2, [`${price}`, lIncome, lIncome, `${vInfoUnlock.id}`]).then(res2 => {
                                ...
                                if (shortPos.length > 0) {
                                    ...
                                    if (sIncome > mIncome && sIncome < sIncomeMax[symbol] * 0.85) {
                                        ...
                                        //更新数据库
                                        let sql2 = 'update `ddlh_coin` set sPrice=?,unLockCount=unLockCount+0.38,unLockIncome=unLockIncome+?,totalIncome=totalIncome+?,lockStatus=0,utime=now() where id=?'
                                        doDb(sql2, [`${price}`, sIncome, sIncome, `${vInfoUnlock.id}`]).then(res2 => {
                                ...
                                if (longPos.length > 0 && shortPos.length > 0) {
                                    ...
                                    if ((totalIncome + vInfoUnlock.unLockIncome - (longPos[0].pp * 4 * vInfoUnlock.unLockCount * 0.01 * 0.04)) > longPos[0].pp * 0.005) {
                                        ...
                                        updatePosAndSymbol(symbol, 'L', 0, price, false)
                                        //更新数据库
                                        let sql2 = 'update `ddlh_coin` set sPrice=?,lPrice=?,unLockCount=1,unLockIncome=0,totalIncome=totalIncome+?,lockStatus=0,utime=now() where id=?'
