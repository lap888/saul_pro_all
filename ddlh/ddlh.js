 ...
 * @Last Modified time: 2023-04-08 09:49:09
 */
...
sclient.createListenKey().then(response => {
    ...
    let sql = "select * from `ddlh_coin` order by ons desc"
    doDb(sql, []).then(dbCoin => {
        ...
        sclient.combinedStreams(SymbolDataArrys, {
            ...
            message: (msg) => {
                ...
                let curSymbol = ''
                //10毫秒过滤
                if (DodoDate[symbol] == undefined || Date.now() - DodoDate[symbol] > 5) {
                    DodoDate[symbol] = Date.now()
                    if (coinInfo.length > 0) {
                        // 实时更新计算持仓价格
                        updateIncome()
                        // 每秒更新持仓
                        if (UpdatePosDate == null || Date.now() - UpdatePosDate > 8000) {
                        ...
                        if (symbol == curSymbol) {
                            ...
                            if (kTimeDate[symbol] == undefined || Date.now() - kTimeDate[symbol] > 5000) {
                            ...
                            if (longPos.length > 0) {
                                ...
                                if (longPos[0].pp < coinInfo[0].totalAmount) {
                                    ...
                                    if (ma90m1[symbol] == 'L' && ma90s1[symbol] == 'L') {
                                        if (coinInfo[0].lRightAddCount < coinInfo[0].rightAddMax) {
                                            ...
                                            if (price > coinInfo[0].lPrice * (1 + 0.001 * coinInfo[0].rightAddBl * coinInfo[0].fkRate) && Date.now() - OpenOrderDate[symbol] > nextOrderWait) {
                                        ...
                                        if (coinInfo[0].lErrorAddCount < coinInfo[0].errorAddMax) {
                                            ...
                                            if (price < coinInfo[0].lPrice * (1 - rate) && price < longPos[0].ep && price > lPriceMin[symbol] * (1 + 0.001 * coinInfo[0].backRate) && Date.now() - OpenOrderDate[symbol] > nextOrderWait) {
                                                OpenOrderDate[symbol] = Date.now()
                                ...
                                //回调止盈 且 覆盖手续费
                                if (((price > longPos[0].ep * (1 + 0.001 * coinInfo[0].rightAddBl) && price < lPriceMax[symbol] * (1 - 0.001 * coinInfo[0].backRate)) || (longPos[0].up > coinInfo[0].lockAmount * 0.68)) && Date.now() - OpenOrderDate[symbol] > nextOrderWait) {
                                    OpenOrderDate[symbol] = Date.now()
                                ...
                                if (longPos[0].up < -(coinInfo[0].lockAmount * coinInfo[0].unLockCount + coinInfo[0].unLockIncome)) {
                                    ...
                                    if (sAmount < longPos[0].pa && Date.now() - OpenOrderDate[symbol] > nextOrderWait) {
                            ...
                            if (shortPos.length > 0) {
                                ...
                                if (shortPos[0].pp < coinInfo[0].totalAmount) {
                                    ...
                                    if (ma90m1[symbol] == 'S' && ma90s1[symbol] == 'S') {
                                        if (coinInfo[0].sRightAddCount < coinInfo[0].rightAddMax) {
                                            ...
                                            if (price < coinInfo[0].sPrice * (1 - 0.001 * coinInfo[0].rightAddBl * coinInfo[0].fkRate) && Date.now() - OpenOrderDate[symbol] > nextOrderWait) {
                                                OpenOrderDate[symbol] = Date.now()
                                                //追击
                                        ...
                                        if (coinInfo[0].sErrorAddCount < coinInfo[0].errorAddMax) {
                                            ...
                                            if (price > coinInfo[0].sPrice * (1 + rate) && price > shortPos[0].ep && price < sPriceMax[symbol] * (1 - 0.001 * coinInfo[0].backRate) && Date.now() - OpenOrderDate[symbol] > nextOrderWait) {
                                                OpenOrderDate[symbol] = Date.now()
                                                let amount = coinInfo[0].fristAmount * 2 ** coinInfo[0].lErrorAddCount;
                                ...
                                //回调止盈 且 覆盖手续费
                                if (((price < shortPos[0].ep * (1 - 0.001 * coinInfo[0].rightAddBl) && price > sPriceMin[symbol] * (1 + 0.001 * coinInfo[0].backRate)) || (shortPos[0].up > coinInfo[0].lockAmount * 0.68)) && Date.now() - OpenOrderDate[symbol] > nextOrderWait) {
                                    OpenOrderDate[symbol] = Date.now()
                                ...
                                if (shortPos[0].up < -(coinInfo[0].lockAmount * coinInfo[0].unLockCount + coinInfo[0].unLockIncome)) {
                                    ...
                                    if (lAmount < shortPos[0].pa && Date.now() - OpenOrderDate[symbol] > nextOrderWait) {
                ...
                if (DodoDate[symbol] == undefined || Date.now() - DodoDate[symbol] > 2 * 1000) {
                    DodoDate[symbol] = Date.now()
                    if (coinInfoUnLock.length > 0) {
                        ...
                        if (lockSymbol == symbol) {
                            ...
                            if (longPos.length > 0) {
                                ...
                                if (lIncome > coinInfoUnLock[0].lockAmount * 1.5) {
                                    //解锁多单
                                    ...
                                    //锁仓当前正在运行的币种
                                    {
                                        ...
                                        if (sAmount < lAmount && Date.now() - OpenOrderDate[coinInfo[0].symbol] > nextOrderWait) {
                                            let amount = lAmount - sAmount
                                            pLog(`${symbol}解锁,${coinInfo[0].symbol} 锁仓,锁仓开空 记录价格:${price} 下单数量:${amount}`)
                                            OpenOrderDate[coinInfo[0].symbol] = Date.now()
                                            sell(client, coinInfo[0].symbol, amount, -1)
                                            // 更新数据库
                                        ...
                                        if (sAmount > lAmount && Date.now() - OpenOrderDate[coinInfo[0].symbol] > nextOrderWait) {
                                            let amount = sAmount - lAmount
                                            pLog(`${symbol}解锁,${coinInfo[0].symbol} 锁仓,锁仓开多 记录价格:${price} 下单数量:${amount}`)
                                            OpenOrderDate[coinInfo[0].symbol] = Date.now()
                                            buy(client, coinInfo[0].symbol, amount, -1)
                                            // 更新数据库
                                            let sql2 = 'update `ddlh_coin` set lockStatus=1,utime=now() where id=?'
                                            doDb(sql2, [`${coinInfo[0].id}`]).then(res2 => {
                                    ...
                                    //更新数据库
                                    let sql2 = 'update `ddlh_coin` set lPrice=?,unLockCount=unLockCount+0.35,unLockIncome=unLockIncome+?,totalIncome=totalIncome+?,lockStatus=0,utime=now() where id=?'
                                    doDb(sql2, [`${price}`, lIncome, lIncome, `${coinInfoUnLock[0].id}`]).then(res2 => {
                            ...
                            if (shortPos.length > 0) {
                                ...
                                if (sIncome > coinInfoUnLock[0].lockAmount * 1.5) {
                                    //解锁空单
                                    ...
                                    //更新数据库
                                    let sql2 = 'update `ddlh_coin` set sPrice=?,unLockCount=unLockCount+0.35,unLockIncome=unLockIncome+?,totalIncome=totalIncome+?,lockStatus=0,utime=now() where id=?'
                                    doDb(sql2, [`${price}`, sIncome, sIncome, `${coinInfoUnLock[0].id}`]).then(res2 => {
                                        console.log('解锁 多单平仓, ok')
                                    }).catch(e => {
                                        console.error('解锁 多单平仓', e)
                                    })
