 ...
 * @Last Modified time: 2023-08-09 08:38:23
 */
...
let minBdRate = 0.0032
let maxBdRate = 0.0072
...
let symbolSetting = {
    'BNBUSDT': { 'rate': 0.52, 'osc': 0, 'doType': 2, 'action': 'N', 'amount': 0, 'doAmount': 10, 'isSet': 0 },
    'KEYUSDT': { 'rate': 0.52, 'osc': 0, 'doType': 2, 'action': 'N', 'amount': 0, 'doAmount': 10, 'isSet': 0 },
}
...
function sendmsg(msg, fromid = '43142195603@chatroom') {
    ...
    caltime.map(t => {
        ...
        }
        {
            if (allCoinNotice[symbol][t]['min'] == undefined || allCoinNotice[symbol][t]['min'] == 0) {
                ...
                allCoinNotice[symbol][t]['time1'] = DateFormat(d2.getTime())
        ...
        lock.acquire('lock-hjfg-bd', function () {
            // 短期内波动
            if (t == '1m' || t == '5m' || t == '1h') {
                if ((allCoinNotice[symbol][t]['max'] - allCoinNotice[symbol][t]['min']) > rPrice[symbol] * 0.03 && t != '15m' && t != '1h') {
                    if (zfRate[`${symbol}-${allCoinNotice[symbol][t]['time']}-001`] == undefined) {
                        zfRate[`${symbol}-${allCoinNotice[symbol][t]['time']}-001`] = '001'
                        // sendmsg(`[月亮]波动预警通知:${symbol}-${t}-内波动超3%\r\n期间最高:${allCoinNotice[symbol][t]['max']} \r\n期间最低:${allCoinNotice[symbol][t]['min']} \r\n当前价:${rPrice[symbol]} \r\n通报时间:${DateFormat()}`)

                    }
                }
                if ((allCoinNotice[symbol][t]['max'] - allCoinNotice[symbol][t]['min']) > rPrice[symbol] * 0.04 && t != '5m' && t != '15m' && t != '1h') {
                    if (zfRate[`${symbol}-${allCoinNotice[symbol][t]['time']}-002`] == undefined) {
                        zfRate[`${symbol}-${allCoinNotice[symbol][t]['time']}-002`] = '002'
                        // sendmsg(`[月亮]波动预警通知:${symbol}-${t}-内波动超过4%\r\n期间最高:${allCoinNotice[symbol][t]['max']} \r\n期间最低:${allCoinNotice[symbol][t]['min']} \r\n当前价:${rPrice[symbol]} \r\n通报时间:${DateFormat()}`)
                        // sendmsg(`[月亮]波动预警通知:${symbol}-${t}-内波动超过4%\r\n期间最高:${allCoinNotice[symbol][t]['max']} \r\n期间最低:${allCoinNotice[symbol][t]['min']} \r\n当前价:${rPrice[symbol]} \r\n通报时间:${DateFormat()}`,'43680387536@chatroom')
                    }
                }
                if ((allCoinNotice[symbol][t]['max'] - allCoinNotice[symbol][t]['min']) > rPrice[symbol] * 0.05 && t != '5m' && t != '15m' && t != '1h') {
                    if (zfRate[`${symbol}-${allCoinNotice[symbol][t]['time']}-003`] == undefined) {
                        zfRate[`${symbol}-${allCoinNotice[symbol][t]['time']}-003`] = '003'
                        sendmsg(`[月亮]波动预警通知:${symbol}-${t}-内波动超过5%\r\n期间最高:${allCoinNotice[symbol][t]['max']} \r\n期间最低:${allCoinNotice[symbol][t]['min']} \r\n当前价:${rPrice[symbol]} \r\n通报时间:${DateFormat()}`)
                        // sendmsg(`[月亮]波动预警通知:${symbol}-${t}-内波动超过5%\r\n期间最高:${allCoinNotice[symbol][t]['max']} \r\n期间最低:${allCoinNotice[symbol][t]['min']} \r\n当前价:${rPrice[symbol]} \r\n通报时间:${DateFormat()}`, '43680387536@chatroom')
                        //查库里是否有次币种 没有则添加
                        let sql = "select * from xq_coin where uId=? and symbol=?"
                        doDb(sql, [myUid, symbol]).then(dbCoin => {
                            ...
                            if (_dbCoin.startsWith('{')) {
                                ...
                                //     }
                                // })
                                let sql2 = 'update `xq_coin` set utime=now() where uId=? and symbol=?'
                                doDb(sql2, [myUid, symbol]).then(dbCoin => {

                                })

                            } else {
                                let sqlinsert = "INSERT INTO `xq_coin` (`uId`,`symbol`) VALUES (?,?);"
                                doDb(sqlinsert, [myUid, symbol]).then(dbCoin => {
                ...
                if ((allCoinNotice[symbol][t]['max'] - allCoinNotice[symbol][t]['min']) > rPrice[symbol] * 0.08 && t != '1h') {
                    if (zfRate[`${symbol}-${allCoinNotice[symbol][t]['time']}-004`] == undefined) {
                        zfRate[`${symbol}-${allCoinNotice[symbol][t]['time']}-004`] = '004'
                        sendmsg(`[月亮]波动预警通知:${symbol}-${t}-内波动超过8%\r\n期间最高:${allCoinNotice[symbol][t]['max']} \r\n期间最低:${allCoinNotice[symbol][t]['min']} \r\n当前价:${rPrice[symbol]} \r\n通报时间:${DateFormat()}`)
                    }
                }
                if ((allCoinNotice[symbol][t]['max'] - allCoinNotice[symbol][t]['min']) > rPrice[symbol] * 0.1 && t != '1h') {
                    if (zfRate[`${symbol}-${allCoinNotice[symbol][t]['time']}-005`] == undefined) {
                        zfRate[`${symbol}-${allCoinNotice[symbol][t]['time']}-005`] = '005'
                        sendmsg(`[月亮]波动预警通知:${symbol}-${t}-内波动超过10%\r\n期间最高:${allCoinNotice[symbol][t]['max']} \r\n期间最低:${allCoinNotice[symbol][t]['min']} \r\n当前价:${rPrice[symbol]} \r\n通报时间:${DateFormat()}`)
                        sendmsg(`[月亮]波动预警通知:${symbol}-${t}-内波动超过10%\r\n期间最高:${allCoinNotice[symbol][t]['max']} \r\n期间最低:${allCoinNotice[symbol][t]['min']} \r\n当前价:${rPrice[symbol]} \r\n通报时间:${DateFormat()}`, '43680387536@chatroom')
                    }
                }
                if ((allCoinNotice[symbol][t]['max'] - allCoinNotice[symbol][t]['min']) > rPrice[symbol] * 0.2) {
                    if (zfRate[`${symbol}-${allCoinNotice[symbol][t]['time']}-005`] == undefined) {
                        zfRate[`${symbol}-${allCoinNotice[symbol][t]['time']}-005`] = '006'
                        sendmsg(`[月亮]波动预警通知:${symbol}-${t}-内波动超过20%\r\n期间最高:${allCoinNotice[symbol][t]['max']} \r\n期间最低:${allCoinNotice[symbol][t]['min']} \r\n当前价:${rPrice[symbol]} \r\n通报时间:${DateFormat()}`)
                        sendmsg(`[月亮]波动预警通知:${symbol}-${t}-内波动超过20%\r\n期间最高:${allCoinNotice[symbol][t]['max']} \r\n期间最低:${allCoinNotice[symbol][t]['min']} \r\n当前价:${rPrice[symbol]} \r\n通报时间:${DateFormat()}`, '43680387536@chatroom')
                    }
                }
            }
...
client.exchangeInfo().then(ex => {
    ...
    exInfo.forEach(e => {
        ...
        quantityPrecision2[e.symbol] = e.quantityPrecision;
        e.filters.map((c) => {
                                    ...
                                    if ((sDt[`${symbol}`] == undefined || Date.now() - sDt[`${symbol}`] > sDtTime * 1000) && (sDt2[`${symbol}`] == undefined || sDt2[`${symbol}`] == -1)) {
                                        ...
                                        if (symbolSetting[`${symbol}`]['amount'] > 0 && symbolSetting[`${symbol}`]['osc'] > 0) {
                                            ...
                                            lock.acquire('lock-m-order', function () {
                                                ...
                                                if (symbolflag[symbol] == 'L' && posNow <= 3) {
                                                    pLog(`${posNow} | ${symbol}=>开仓${symbolflag[symbol] == 'S' ? '做空' : '做多'} | 价格${rPrice[symbol]} | 数量${symbolSetting[`${symbol}`]['amount']} | ${interValTime}秒内,价格波动:${symbolSetting[`${symbol}`]['osc'] * 100 / price}% | ${btPrice}U | doType:${symbolSetting[`${symbol}`]['doType']} | doAction:${symbolSetting[`${symbol}`]['action']} 休息${sDtTime}秒`)
                                                    sendmsg(`[太阳]${symbol}=>开仓${symbolflag[symbol] == 'S' ? '做空' : '做多'} \r\n价格${rPrice[symbol]} \r\n数量${symbolSetting[`${symbol}`]['amount']} \r\n${interValTime}秒内,价格波动:${_N(symbolSetting[`${symbol}`]['osc'] * 100 / price, 4)}% ≈ ${btPrice}U \r\n开单方向:${symbolSetting[`${symbol}`]['doType'] == 1 ? '顺势' : '逆势'}`)
                                                    posNow++
                                                ...
                                                if (symbolflag[symbol] == 'S' && posNow <= 3) {
                                                    pLog(`${posNow} | ${symbol}=>开仓${symbolflag[symbol]} | 价格${rPrice[symbol]} | 数量${symbolSetting[`${symbol}`]['amount']} | ${interValTime}秒内,价格波动:${symbolSetting[`${symbol}`]['osc'] * 100 / price}% | ${btPrice}U | doType:${symbolSetting[`${symbol}`]['doType'] == 1 ? '顺势' : '逆势'} | doAction:${symbolSetting[`${symbol}`]['action']} 休息${sDtTime}秒`)
                                                    sendmsg(`[太阳]${symbol}=>开仓${symbolflag[symbol] == 'S' ? '做空' : '做多'} \r\n价格${rPrice[symbol]} \r\n数量${symbolSetting[`${symbol}`]['amount']} \r\n${interValTime}秒内,价格波动:${_N(symbolSetting[`${symbol}`]['osc'] * 100 / price, 4)}% ≈ ${btPrice}U \r\n开单方向:${symbolSetting[`${symbol}`]['doType'] == 1 ? '顺势' : '逆势'}`)
                                                    posNow++
                                                    sDt[`${symbol}`] = Date.now()//开仓
                                                    sDt2[`${symbol}`] = Date.now()//平仓
