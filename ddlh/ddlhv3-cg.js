...
const apiKey = '1tWYkmhUCgDw3ER0O3f9DaMjaJSUYxoBKudWu6mDMUF9fC6iqoxG1zulWJttnFn1'
const apiSecret = 'UKlTRemE80v6sHcBWcYcTnX7RIBSDvSHpZ6dqLBHOhLEhRdU5QEU1IL19stdYAG1'
...
client.listenKeyPost().then(response => {
    ...
    let sql = "select * from `ddlh_coin` where uId=1002 order by ons desc"
                    ...
                    if (coinInfo.length > 0) {
                        ...
                        if (symbol == curSymbol) {
                            ...
                            if (longPos.length > 0 && longPos[0].pa != 0 && (Date.now() - OpenForderDate[symbol] > 500 || OpenForderDate[symbol] == undefined)) {
                                ...
                                if (longPos[0].pa > 0 && longPos[0].up < lIncomeMax[symbol] * 0.9 && price > longPos[0].ep * (1 + 0.001 * 2.35) && (Date.now() - OpenOrderDate0[symbol] > nextOrderWait || OpenOrderDate0[symbol] == undefined)) {
                                    OpenOrderDate0[symbol] = Date.now()
                                ...
                                if (longPos[0].up < -(coinInfo[0].lockAmount * coinInfo[0].unLockCount + coinInfo[0].unLockIncome) && (Date.now() - OpenOrderDate0[symbol] > nextOrderWait || OpenOrderDate0[symbol] == undefined)) {
                                    ...
                                    if (sAmount < doAmount) {
                                        ...
                                        amount = _N(amount, numLength)
                                        pLog(`${symbol} 多单浮亏达标，锁仓开空 记录价格:${price} 下单数量:${amount}`)
                                        OpenOrderDate0[symbol] = Date.now()
                                        ...
                                        // 更新数据库
                                        let addcount1 = Math.max(coinInfo[0].lErrorAddCount, coinInfo[0].sErrorAddCount)
                                        let addcount2 = Math.max(coinInfo[0].lRightAddCount, coinInfo[0].sRightAddCount)
                                        let sql2 = 'update `ddlh_coin` set sPrice=?,lockStatus=1,lErrorAddCount=?,sErrorAddCount=?,lRightAddCount=?,sRightAddCount=?,utime=now() where id=?'
                                        doDb(sql2, [`${price}`, addcount1, addcount1, addcount2, addcount2, `${coinInfo[0].id}`]).then(res2 => {
                                ...
                                if ((OpenForderDate[symbol] == undefined || Date.now() - OpenForderDate[symbol] > 500) && (longPos.length <= 0 || (longPos.length > 0 && longPos[0].pa == 0))) {
                                    ...
                                    if (ma90m1[symbol] == 'L' && ma90s1[symbol] == 'L') {
                                        ...
                                        pLog(`${symbol}首单 多单开仓 下单记录价格:${price} 下单数量:${coinInfo[0].fristAmount}`)
                            ...
                            if (shortPos.length > 0 && shortPos[0].pa != 0 && (Date.now() - OpenForderDate[symbol] > 500 || OpenForderDate[symbol] == undefined)) {
                                ...
                                if (shortPos[0].up < -(coinInfo[0].lockAmount * coinInfo[0].unLockCount + coinInfo[0].unLockIncome) && (Date.now() - OpenOrderDate1[symbol] > nextOrderWait || OpenOrderDate1[symbol] == undefined)) {
                                    ...
                                    if (lAmount < doAmount) {
                                        ...
                                        let addcount1 = Math.max(coinInfo[0].lErrorAddCount, coinInfo[0].sErrorAddCount)
                                        let addcount2 = Math.max(coinInfo[0].lRightAddCount, coinInfo[0].sRightAddCount)
                                        let sql2 = 'update `ddlh_coin` set lPrice=?,lockStatus=1,lErrorAddCount=?,sErrorAddCount=?,lRightAddCount=?,sRightAddCount=?,utime=now() where id=?'
                                        doDb(sql2, [`${price}`, addcount1, addcount1, addcount2, addcount2, `${coinInfo[0].id}`]).then(res2 => {
                        ...
                        coinInfoUnLock.forEach(vInfoUnlock => {
                            ...
                            if (lockSymbol == symbol) {
                                ...
                                if (longPos.length > 0) {
                                    ...
                                    //大于持仓价值既可以解锁
                                    let mIncome = 0
                                    let mPp = longPos[0].pp * 0.01
                                    mIncome = vInfoUnlock.lockAmount < mPp ? vInfoUnlock.lockAmount : mPp
                                    if (lIncome + vInfoUnlock.gIncome > mIncome && lIncome < lIncomeMax[symbol] * 0.9) {
                                        //解锁多单
                                        ...
                                        //更新数据库
                                        let sql2 = 'update `ddlh_coin` set lPrice=?,unLockCount=unLockCount+0.5,gIncome=0,unLockIncome=unLockIncome+?,totalIncome=totalIncome+?,lockStatus=0,utime=now() where id=?'
                                ...
                                if (shortPos.length > 0) {
                                    ...
                                    if (sIncome + vInfoUnlock.gIncome > mIncome && sIncome < sIncomeMax[symbol] * 0.9) {
                                        //解锁空单
                                        ...
                                        //更新数据库
                                        let sql2 = 'update `ddlh_coin` set sPrice=?,unLockCount=unLockCount+0.5,gIncome=0,unLockIncome=unLockIncome+?,totalIncome=totalIncome+?,lockStatus=0,utime=now() where id=?'
                                        doDb(sql2, [`${price}`, sIncome * 0.99, sIncome, `${vInfoUnlock.id}`]).then(res2 => {
                                        ...
                                        if (coinInfo.length > 0) {
                                            ...
                                            let doAmountS = Math.min(sAmount, maxAmount)
                                            let islock = 0
                                            if (sInfo[0].up < -vInfoUnlock.lockAmount || lInfo[0].up < -vInfoUnlock.lockAmount) {
                                                islock = 1
                                            }
                                            if (sAmount < doAmountL && (Date.now() - OpenOrderDateSc1[coinInfo[0].symbol] > nextOrderWait || OpenOrderDateSc1[coinInfo[0].symbol] == undefined)) {
                                                let amount = _N(doAmountL - sAmount, numLength)
                                                pLog(`${symbol}解锁,${coinInfo[0].symbol} 锁仓=>${islock},锁仓开空 记录价格:${price} 下单数量:${amount}`)
                                                ...
                                                updatePosAndSymbol(coinInfo[0].symbol, 'S', amount, price, false, true)
                                                // 更新数据库
                                                let sql2 = 'update `ddlh_coin` set lockStatus=?,lErrorAddCount=?,sErrorAddCount=?,lRightAddCount=?,sRightAddCount=?,utime=now() where id=?'
                                                doDb(sql2, [islock, addcount1, addcount1, addcount2, addcount2, `${coinInfo[0].id}`]).then(res2 => {
                                                    console.log(`${symbol}解锁,${coinInfo[0].symbol} 锁仓锁仓开空，更新数据库, ok`)
                                                }).catch(e => {
                                                    console.error(`${symbol}解锁,${coinInfo[0].symbol} 锁仓锁仓开空，更新数据库, ok`, e)
                                                })
                                            ...
                                            if (doAmountS > lAmount && (Date.now() - OpenOrderDateSc1[coinInfo[0].symbol] > nextOrderWait || OpenOrderDateSc1[coinInfo[0].symbol] == undefined)) {
                                                let amount = _N(doAmountS - lAmount, numLength)
                                                pLog(`${symbol}解锁,${coinInfo[0].symbol} 锁仓=>${islock},锁仓开多 记录价格:${price} 下单数量:${amount}`)
                                                OpenOrderDateSc1[coinInfo[0].symbol] = Date.now()
                                                buy(client, coinInfo[0].symbol, amount, -1).catch(e => {
                                                    console.log(coinInfo[0].symbol, e)
                                                })
                                ...
                                if ((longPos.length > 0 && longPos[0].pa != 0) && (shortPos.length > 0 && shortPos[0].pa != 0)) {
                                    ...
                                    if ((totalIncome + vInfoUnlock.unLockIncome + vInfoUnlock.gIncome - (longPos[0].pp * 4 * vInfoUnlock.unLockCount * 0.01 * 0.04)) > longPos[0].pp * 0.001) {
                                        //持平清仓
                                        ...
                                        updatePosAndSymbol(symbol, 'L', 0, price, false)
                                        //更新数据库
                                        let sql2 = 'update `ddlh_coin` set sPrice=?,lPrice=?,unLockCount=1,gIncome=0,gCount=0,gPrice=0,unLockIncome=0,totalIncome=totalIncome+?,lockStatus=0,utime=now() where id=?'
                                    ...
                                    // 网格磨                                   
                                    if (Math.abs(vInfoUnlock.gCount) < vInfoUnlock.rightAddMax && vInfoUnlock.isGrid == 1 && shortPos[0].pa > vInfoUnlock.rightPpAmount && longPos[0].pa > vInfoUnlock.rightPpAmount) {
                                        ...
                                        if (price > vInfoUnlock.gPrice * (1 + vInfoUnlock.gRate * 0.001) && price < lPriceMax[symbol] * (1 - 0.001)) {
                                            ...
                                            doDb(sql2, [`${price}`, income1 + income2 - fee, income1 + income2 - fee, `${vInfoUnlock.id}`]).then(res2 => {
                                                pLog(`价格:${price},高于gPrice:${oldGPrice},网格距离:${vInfoUnlock.gRate} 数量:${amount},多单收益:${income1}，空单收益:${income2}，手续费:${fee}，发生回调多单止盈 空单加仓, ok`)
                                            }).catch(e => {
                                        ...
                                        //价格低于gPrice网格距离 发生回调空单止盈 多单加仓 && sIncome < sIncomeMax[symbol] * 0.9
                                        if (price < vInfoUnlock.gPrice * (1 - vInfoUnlock.gRate * 0.001) && price > sPriceMin[symbol] * (1 + 0.001)) {
                                            ...
                                            doDb(sql2, [`${price}`, income1 + income2 - fee, income1 + income2 - fee, `${vInfoUnlock.id}`]).then(res2 => {
                                                pLog(`价格:${price},低于gPrice:${oldGPrice},网格距离:${vInfoUnlock.gRate} 数量:${amount},多单收益:${income1}，空单收益:${income2}，手续费:${fee}，发生回调空单止盈 多单加仓, ok`)
                                            }).catch(e => {
