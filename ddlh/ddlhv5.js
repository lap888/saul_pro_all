 ...
 * @Last Modified time: 2023-05-11 10:56:05
 */
                        ...
                        coinInfo.forEach(vInfo => {
                            ...
                            if (symbol == curSymbol) {
                                ...
                                if (longPos.length > 0 && longPos[0].pa != 0 && (Date.now() - OpenForderDate[symbol] > 500 || OpenForderDate[symbol] == undefined)) {
                                    ...
                                    //浮亏锁仓 + 解锁收益
                                    let nn = Math.max(longPos[0].pp / 2 * 0.01, vInfo.lockAmount)
                                    if (longPos[0].up + vInfo.unLockIncome < -nn * vInfo.unLockCount && (Date.now() - OpenOrderDate0[symbol] > 1000 * 5 || OpenOrderDate0[symbol] == undefined)) {
                                        ...
                                        if (sAmount < doAmount) {
                                            ...
                                            amount = _N(amount, numLength)
                                            pLog(`${symbol} 多单浮亏${longPos[0].up}达标，锁仓开空 nn=${nn}|unLockCount=${vInfo.unLockCount}|lockAmount=${vInfo.lockAmount} 记录价格:${price} 下单数量:${amount}`)
                                            OpenOrderDate0[symbol] = Date.now()
                                ...
                                if (shortPos.length > 0 && shortPos[0].pa != 0 && (Date.now() - OpenForderDate[symbol] > 500 || OpenForderDate[symbol] == undefined)) {
                                    ...
                                    //浮亏锁仓 + 解锁收益
                                    let nn = Math.max(shortPos[0].pp / 2 * 0.01, vInfo.lockAmount)
                                    if (shortPos[0].up + vInfo.unLockIncome < -nn * vInfo.unLockCount && (Date.now() - OpenOrderDate1[symbol] > 1000 * 5 || OpenOrderDate1[symbol] == undefined)) {
                                        ...
                                        if (lAmount < doAmount) {
                                            ...
                                            amount = _N(amount, numLength)
                                            pLog(`${symbol} 空单浮亏${shortPos[0].up}达标锁仓 nn=${nn}|unLockCount=${vInfo.unLockCount}|lockAmount=${vInfo.lockAmount}，开多 记录价格:${price} 下单数量:${amount}`)
                                            OpenOrderDate1[symbol] = Date.now()
                                            buy(client, symbol, amount, -1).catch(e => {
                                                console.log(symbol, e)
                                            })
                        ...
                        coinInfoUnLock.forEach(vInfoUnlock => {
                            ...
                            if (lockSymbol == symbol) {
                                ...
                                if (longPos.length > 0) {
                                    ...
                                    //大于持仓价值既可以解锁
                                    let mIncome = 0
                                    let mPp = longPos[0].pp * 0.0083
                                    mIncome = vInfoUnlock.lockAmount < mPp ? vInfoUnlock.lockAmount : mPp
                                    if (lIncome + vInfoUnlock.gIncome > mIncome && lIncome < lIncomeMax[symbol] * 0.764) {
                                        OpenOrderDate1[symbol] = Date.now()
                                        //解锁多单
                                        pLog(`${symbol} 解锁 多单收益有回调 多单平仓 记录价格:${price} 收益:${lIncome} 数量:${longPos[0].pa}`)
                                        buy_close(client, symbol, longPos[0].pa, -1).catch(e => {
                                            console.log(symbol, e)
                                        })
                                        updatePosAndSymbol(symbol, 'L', 0, price, true, false, mIncome * 0.9)
                                        let addSql = ''
                                        if (vInfoUnlock.whoLock == 'L') {
                                            addSql = 'unLockIncome=0,unLockCount=1,whoLock=0,'
                                        }
                                        //更新数据库
                                        let sql2 = `update ddlh_coin set lPrice=?,unLockCount=unLockCount+0.382,lRightAddCount=0,lErrorAddCount=0,gIncome=0,gCount=0,unLockIncome=unLockIncome+?,totalIncome=totalIncome+?,lockStatus=0,${addSql}utime=now() where id=?`
                                        doDb(sql2, [`${price}`, lIncome * 0.9, lIncome, `${vInfoUnlock.id}`]).then(res2 => {
                                ...
                                if (shortPos.length > 0) {
                                    ...
                                    let mIncome = 0
                                    let mPp = shortPos[0].pp * 0.0083
                                    mIncome = vInfoUnlock.lockAmount < mPp ? vInfoUnlock.lockAmount : mPp
                                    let sIncome = shortPos[0].up
                                    printIncomeInfo += ` | 空单收益:${sIncome}`
                                    if (sIncome + vInfoUnlock.gIncome > mIncome && sIncome < sIncomeMax[symbol] * 0.764) {
                                        OpenOrderDate0[symbol] = Date.now()
                                        //解锁空单
                                        ...
                                        updatePosAndSymbol(symbol, 'S', 0, price, true, false, mIncome * 0.9)
                                        let addSql = ''
                                        if (vInfoUnlock.whoLock == 'S') {
                                            addSql = 'unLockIncome=0,unLockCount=1,whoLock=0,'
                                        }
                                        //更新数据库
                                        let sql2 = `update ddlh_coin set sPrice=?,unLockCount=unLockCount+0.382,sRightAddCount=0,sErrorAddCount=0,gIncome=0,gCount=0,unLockIncome=unLockIncome+?,totalIncome=totalIncome+?,lockStatus=0,${addSql}utime=now() where id=?`
                                        doDb(sql2, [`${price}`, sIncome * 0.9, sIncome, `${vInfoUnlock.id}`]).then(res2 => {
