    ...
    symbolDatas.forEach((v, i) => {
        if (v.symbol == symbol) {
            if (action == 'LONG') {
                ...
                if (isJs) {
                    symbolDatas[i].unLockCount = symbolDatas[i].unLockCount + 0.33
...
client.listenKeyPost().then(response => {
    ...
    let sql = `select * from ddlh_coin where uId=${uids} order by ons desc`
                        ...
                        coinInfo.forEach(vInfo => {
                            ...
                            if (symbol == curSymbol) {
                                ...
                                if (longPos.length > 0 && longPos[0].pa != 0 && (Date.now() - OpenForderDate[symbol] > 500 || OpenForderDate[symbol] == undefined)) {
                                    ...
                                    //回调止盈 且 覆盖手续费 
                                    if (longPos[0].pa > 0 && longPos[0].up < lIncomeMax[symbol] * 0.82 && price > longPos[0].ep * (1 + 0.001 * 2.65) && (Date.now() - OpenOrderDate0[symbol] > nextOrderWait || OpenOrderDate0[symbol] == undefined)) {
                                        OpenOrderDate0[symbol] = Date.now()
                                    ...
                                    if (longPos[0].up + vInfo.unLockIncome < -vInfo.lockAmount * vInfo.unLockCount && (Date.now() - OpenOrderDate0[symbol] > nextOrderWait || OpenOrderDate0[symbol] == undefined)) {
                                        ...
                                        if (sAmount < doAmount) {
                                            ...
                                            doDb(sql2, [`${price}`, addcount1, addcount1, addcount2, addcount2, `${vInfo.id}`]).then(res2 => {
                                ...
                                if (shortPos.length > 0 && shortPos[0].pa != 0 && (Date.now() - OpenForderDate[symbol] > 500 || OpenForderDate[symbol] == undefined)) {
                                    ...
                                    //回调止盈 且 覆盖手续费
                                    if (shortPos[0].pa > 0 && (shortPos[0].up < sIncomeMax[symbol] * 0.82 || price > sPriceMin[symbol] * (1 + 0.001 * vInfo.backRate)) && price < shortPos[0].ep * (1 - 0.001 * 2.65) && (Date.now() - OpenOrderDate1[symbol] > nextOrderWait || OpenOrderDate1[symbol] == undefined)) {
                                        OpenOrderDate1[symbol] = Date.now()
                            ...
                            if (lockSymbol == symbol) {
                                ...
                                if (longPos.length > 0) {
                                    ...
                                    if (lIncome + vInfoUnlock.gIncome > mIncome && lIncome < lIncomeMax[symbol] * 0.9) {
                                        ...
                                        //更新数据库
                                        let sql2 = 'update `ddlh_coin` set lPrice=?,unLockCount=unLockCount+0.33,gIncome=0,gCount=0,unLockIncome=unLockIncome+?,totalIncome=totalIncome+?,lockStatus=0,utime=now() where id=?'
                                ...
                                if (shortPos.length > 0) {
                                    ...
                                    if (sIncome + vInfoUnlock.gIncome > mIncome && sIncome < sIncomeMax[symbol] * 0.9) {
                                        ...
                                        //更新数据库
                                        let sql2 = 'update `ddlh_coin` set sPrice=?,unLockCount=unLockCount+0.33,gIncome=0,gCount=0,unLockIncome=unLockIncome+?,totalIncome=totalIncome+?,lockStatus=0,utime=now() where id=?'
                                ...
                                if (((longPos.length > 0 && longPos[0]?.pa > 0) && (shortPos.length <= 0 || shortPos[0]?.pa == 0)) || ((longPos.length <= 0 || longPos[0]?.pa == 0) && (shortPos.length > 0 && shortPos[0]?.pa > 0))) {
                                    let sql2 = 'update `ddlh_coin` set lockStatus=0,gIncome=0,unLockIncome=0,utime=now() where id=?'
