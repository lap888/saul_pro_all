 ...
 * @Last Modified time: 2023-02-21 11:29:19
 */
...
const UserInfo = async (rToken) => {
    let res = {}
    let _res = await axios.post(`https://gw.rebiasia.com/hk-web/v2/getUserInfo?token=${rToken}&platform=1&client=1&deviceId=${randomString()}&versionCode=3.0.1&lang=zh_CN`, { "token": rToken, "platform": 1, "client": 1, "deviceId": randomString(), "versionCode": "3.0.1", "lang": "zh_CN" });
    res = _res['data']
    console.log(`扫码|查询 用户信息`, res)
    return res;
}
...
const getHistoryOrderList = async (pageNo, pageSize, rToken) => {
    let res = {}
    let _res = await axios.post(`https://gw.rebiasia.com/swap/v2/perpetual/products/history-list?indexBase=&quote=&orderType=0&pageSize=${pageSize}&page=${pageNo}&token=${rToken}&platform=1&client=1&deviceId=${randomString()}&versionCode=3.0.1&lang=zh_CN`);
    res = _res['data']
...
// let token='eyJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJob3Rjb2luIiwidWlkIjoidmxOaHM5K2s2bWp4SGVrR0tMQllhQT09Iiwic291cmNlIjoiTlVqaUVWREJLRndnVVY4K3dQT1FLQT09IiwiaWF0IjoxNjc2OTQ1NDkxLCJqdGkiOiI0M2U2MzFlOC1iMzc1LTRiMDQtODE2Mi04MGJiODZlMTBlYjUiLCJpcCI6IllCay9tNDE2cVFySlQxZE4yckRYUXc9PSJ9.knahKvBW6MDB2pB3j4KmEm1YE1KmvXQpea182_SEz4k$eyJhbGciOiJIUzI1NiJ9..iLZmYlUamplsTRM0GsjDkP6uspL_pXA8t1bNKgX24Wk'
// UserInfo(token)
// Assets('haha', token)
