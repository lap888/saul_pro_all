 ...
 * @Last Modified time: 2023-01-06 21:05:17
 */
...
const { SubscribeReq, BitgetWsClient } = require('bitget-openapi');
...
let btRate = { 'ETHUSDT': _btr + 0.0003, 'ATOMUSDT': _btr, 'BTCUSDT': _btr + 0.0001, 'LTCUSDT': _btr, 'XRPUSDT': _btr + 0.0001, 'ADAUSDT': _btr + 0.0001, 'ETCUSDT': _btr + 0.0003, 'BCHUSDT': _btr + 0.0002, 'MATICUSDT': _btr, 'LINKUSDT': _btr + 0.0002, 'DOGEUSDT': _btr + 0.0002 }
...
bitgetWsClient.on('close', () => {
    ...
    bitgetWsClient.on('open', () => {
        ...
        timer = setInterval(() => {
            bitgetWsClient.send("ping")
            console.log('2', Object.entries(bgP)[0])
        }, 20 * 1000);
    })
})
        ...
        message: (msg) => {
            ...
            if (twoSymbolData[symbol].t == null || Date.now() - twoSymbolData[symbol].t <= interValTime * 1000) {
                ...
                if (symbolAction[`${symbol}`] != undefined && symbolAction[`${symbol}`].flag == 1) {
                    if (Date.now() - sDt4[symbol] > 0.3 * 1000) {
                        ...
                        let as = []
                        getDealRecordList(1, 20, sT, eT, vv.name, vv.rToken, vv.btSession, vv.btNewSession).then(res => {
                            if (res.code == "200") {
                                let ac = vv.action == 'S' ? '开空' : '开多'
                                res.data.dealRecordList.map(v => {
                                    if (v['symbolDisplayName'] == vv.symbol && (v['delegateTypeDesc'] == ac)) {
                                        as.push(v)
                                    }
                                })
                                if (as.length > 0 && Number(as[0]['createTime']) > vv.time) {
                                    symbolAction[`${symbol}`].flag = -1
                                    symbolAction[`${symbol}`].price = Number(as[0]['positionAverage'])
                ...
                //平仓
                if (sDt2[`${symbol}`] != -1 && sDt2[`${symbol}`] != undefined && Date.now() - sDt[`${symbol}`] > 45 * 1000) {
                    ...
                    //止损&止盈
                    if (sDt3[symbol] == null || Date.now() - sDt3[symbol] > 0.15 * 1000) {
                        sDt3[symbol] = Date.now();
                        let btPrice2 = _N(btRate[`${symbol}`] * Number(price), 8);
                        let btPrice3 = btPrice2 * 0.4;
                        let oneP = btPrice2 / 10;
                        ExKey.map(v => {
                            let amount = v[symbol]
                            if (isClose[`${v.name}_${symbol}`] == 1) {
                                if (symbolAction[`${symbol}`].action == 'S') {
                                    if (bgP[symbol] > symbolAction[`${symbol}`].price + btPrice3) {
                                        sDt2[`${symbol}`] = -1;
                                        SellClose(symbol, amount, v.rToken, v.btSession, v.btNewSession).then(res => {
                                            if (res.code == "200") {
                                    ...
                                    } else if (bgP[symbol] < symbolAction[`${symbol}`].price - btPrice2) {
                                        sDt2[`${symbol}`] = -1;
                                        SellClose(symbol, amount, v.rToken, v.btSession, v.btNewSession).then(res => {
                                            if (res.code == "200") {
                                                pLog(`${v.name} | 纯秒K | ${symbol} | 止盈 | 平空 | 币安价格:${price} | bg价格:${bgP[symbol]} | 持仓均价:${symbolAction[`${symbol}`].price}`)
                                                isClose[`${v.name}_${symbol}`] = -1
                                            } else {
                                        ...
                                        //分段止盈
                                        if (threeSymbolData[symbol].min < symbolAction[`${symbol}`].price - oneP * 8 && bgP[symbol] > symbolAction[`${symbol}`].price - oneP * 7) {
                                            sDt2[`${symbol}`] = -1;
                                            SellClose(symbol, amount, v.rToken, v.btSession, v.btNewSession).then(res => {
                                                if (res.code == "200") {
                                        ...
                                        } else if (threeSymbolData[symbol].min < symbolAction[`${symbol}`].price - oneP * 6 && bgP[symbol] > symbolAction[`${symbol}`].price - oneP * 5) {
                                            sDt2[`${symbol}`] = -1;
                                            SellClose(symbol, amount, v.rToken, v.btSession, v.btNewSession).then(res => {
                                                if (res.code == "200") {
                                        ...
                                        } else if (threeSymbolData[symbol].min < symbolAction[`${symbol}`].price - oneP * 4 && bgP[symbol] > symbolAction[`${symbol}`].price - oneP * 3) {
                                            sDt2[`${symbol}`] = -1;
                                            SellClose(symbol, amount, v.rToken, v.btSession, v.btNewSession).then(res => {
                                                if (res.code == "200") {
                                        ...
                                        } else if (threeSymbolData[symbol].min < symbolAction[`${symbol}`].price - oneP * 2 && bgP[symbol] > symbolAction[`${symbol}`].price - oneP * 1) {
                                            sDt2[`${symbol}`] = -1;
                                            SellClose(symbol, amount, v.rToken, v.btSession, v.btNewSession).then(res => {
                                                if (res.code == "200") {
                                                    pLog(`${v.name} | 纯秒K | ${symbol} | 第一段止盈 | 平空 | 币安价格:${price} | bg价格:${bgP[symbol]} | 持仓均价:${symbolAction[`${symbol}`].price}`)
                                                    isClose[`${v.name}_${symbol}`] = -1
                                                } else {
                                                    console.log(`${v.name}_${symbol}`, '止盈|空单平空失败|1', res)
                                                }
                                ...
                                if (symbolAction[`${symbol}`].action == 'L') {
                                    if (bgP[symbol] < symbolAction[`${symbol}`].price - btPrice3) {
                                        ...
                                        BuyClose(symbol, amount, v.rToken, v.btSession, v.btNewSession).then(res => {
                                            if (res.code == "200") {
                                                pLog(`${v.name} | 纯秒K | ${symbol} | 止盈 | 平多 | 币安价格:${price} | bg价格:${bgP[symbol]} | 持仓均价:${symbolAction[`${symbol}`].price}`)
                                                isClose[`${v.name}_${symbol}`] = -1
                                            } else {
                                        ...
                                        if (threeSymbolData[symbol].max > symbolAction[`${symbol}`].price + oneP * 8 && bgP[symbol] < symbolAction[`${symbol}`].price + oneP * 7) {
                                            sDt2[`${symbol}`] = -1;
                                            BuyClose(symbol, amount, v.rToken, v.btSession, v.btNewSession).then(res => {
                                                if (res.code == "200") {
                                        ...
                                        } else if (threeSymbolData[symbol].max > symbolAction[`${symbol}`].price + oneP * 6 && bgP[symbol] < symbolAction[`${symbol}`].price + oneP * 5) {
                                            sDt2[`${symbol}`] = -1;
                                            BuyClose(symbol, amount, v.rToken, v.btSession, v.btNewSession).then(res => {
                                                if (res.code == "200") {
                                        ...
                                        } else if (threeSymbolData[symbol].max > symbolAction[`${symbol}`].price + oneP * 4 && bgP[symbol] < symbolAction[`${symbol}`].price + oneP * 3) {
                                            sDt2[`${symbol}`] = -1;
                                            BuyClose(symbol, amount, v.rToken, v.btSession, v.btNewSession).then(res => {
                                                if (res.code == "200") {
                                        ...
                                        } else if (threeSymbolData[symbol].max > symbolAction[`${symbol}`].price + oneP * 2 && bgP[symbol] < symbolAction[`${symbol}`].price + oneP * 1) {
                                            sDt2[`${symbol}`] = -1;
                                            BuyClose(symbol, amount, v.rToken, v.btSession, v.btNewSession).then(res => {
                                                if (res.code == "200") {
                                                    pLog(`${v.name} | 纯秒K | ${symbol} | 第一段止盈 | 平多 | 币安价格:${price} | bg价格:${bgP[symbol]} | 持仓均价:${symbolAction[`${symbol}`].price}`)
                                                    isClose[`${v.name}_${symbol}`] = -1
                                                } else {
                                                    console.log(`${v.name}_${symbol}`, '止盈|多单平多失败|1', res)
                                                }
                ...
                if (diffPrice > btPrice1 && ms <= interValTime * 1000 + 20) {
                    if ((sDt[`${symbol}`] == undefined || Date.now() - sDt[`${symbol}`] > sDtTime * 1000) && (sDt2[`${symbol}`] == undefined || sDt2[`${symbol}`] == -1)) {
                        ...
                        symbolAction[`${symbol}`] = { symbol: symbol, action: flag, price: bgP[symbol], time: new Date().getTime(), flag: -1, rToken: '', btSession: '', btNewSession: '' };
                        sDt3[symbol] = Date.now();
                                ...
                                if (openPos[v.name] < 1 && amount != undefined) {
                                    BuyByQ(symbol, amount, v.rToken, v.btSession, v.btNewSession).then(res => {
                                        if (res.code == "200") {
                                            ...
                                            isClose[`${v.name}_${symbol}`] = 1
                                            symbolAction[`${symbol}`].price = (symbolAction[`${symbol}`].price + bgP[symbol]) / 2
