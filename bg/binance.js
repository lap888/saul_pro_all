...
function doSelfRate(symbol, ktime = '1m', limit = 100) {
    client.records(symbol, ktime, limit).then(record => {
        let _ma14 = TA.MA(record, 14)
        let ma14_1 = _N(_ma14[_ma14.length - 2], 8)
        let ma14_2 = _N(_ma14[_ma14.length - 3], 8)
        let ma14_3 = _N(_ma14[_ma14.length - 4], 8)
        let ma14_4 = _N(_ma14[_ma14.length - 5], 8)

        // let _ma30 = TA.MA(record, 30)
        // let ma30_1 = _N(_ma30[_ma30.length - 2], 8)
        // let ma30_2 = _N(_ma30[_ma30.length - 3], 8)
        // let ma30_3 = _N(_ma30[_ma30.length - 4], 8)
        // let ma30_4 = _N(_ma30[_ma30.length - 5], 8)

        // let _ma60 = TA.MA(record, 60)
        // let ma60_1 = _N(_ma60[_ma60.length - 2], 8)
        // let ma60_2 = _N(_ma60[_ma60.length - 3], 8)
        // let ma60_3 = _N(_ma60[_ma60.length - 4], 8)
        // let ma60_4 = _N(_ma60[_ma60.length - 5], 8)
        // let ma99 = TA.MA(record, 99)
        ...
        // let ma99_4 = _N(ma99[ma99.length - 5], 8)
        //第一种指标算法
        let l = (ma14_1 > ma14_3 && ma14_2 > ma14_4 && Math.abs(ma14_1 - ma14_3) > Math.abs(ma14_2 - ma14_4))
        let s = (ma14_1 < ma14_3 && ma14_2 < ma14_4 && Math.abs(ma14_1 - ma14_3) > Math.abs(ma14_2 - ma14_4))
        if (l) {
            cDo[`${symbol}`] = 'L'
        } else if (s) {
            cDo[`${symbol}`] = 'S'
...
async function updateL() {
    //确定杠杆
    let res = await client.leverageBracket()
    for (let i = 0; i < res.data.length; i++) {
        let v = res.data[i];
        let leverage = 20;
        if (symbolDatas.indexOf(v.symbol) != -1) {
            leverage = v.brackets[0].initialLeverage
            // console.log(leverage, v.symbol)
            // await client.leverage(v.symbol, leverage);
            console.log(`修改杠杆:${v.symbol},杠杆:${leverage}`)
            await Sleep(100)
        }
    }
}
            ...
            message: (msg) => {
                ...
                if (dt[`${symbol}`] == undefined || Date.now() - dt[`${symbol}`] > interValTime * 1000) {
                    ...
                    if (symbolPrice[`${symbol}_max`] - symbolPrice[`${symbol}_min`] > btPrice1) {
                        ...
                        //发送开单信号 休息N分钟
                        if ((action == flag || action == '') && cDo[`${symbol}`] == flag) {
