 ...
 * @Last Modified time: 2022-12-25 11:51:05
 */
...
const { BuyClose, BuyByQ, SellByQ, SellClose, Assets, UserInfo } = require('./order')
            ...
            if (twoSymbolData[symbol].t == null || Date.now() - twoSymbolData[symbol].t <= interValTime * 1000) {
                ...
                if (sDt2[`${symbol}`] != -1 && sDt2[`${symbol}`] != undefined && Date.now() - sDt[`${symbol}`] > 1.5 * 1000) {
                    if (symbolAction[`${symbol}`].action == 'S') {
                        ...
                        sDt2[`${symbol}`] = -1;
                        sDt3 = Date.now();
                        pLog(`定时爆破 | 纯秒K | ${symbol} | ${isIncome ? '止盈' : '止损'} | 平空 | 币安价格:${price} | bg价格:${bgEthPrice} | K数据:${JSON.stringify(twoSymbolData[symbol].n)}`)
                        ExKey.map(v => {
                            if (isClose[v.name] == 1) {
                                SellClose(symbol, v.amount, v.rToken, v.btSession, v.btNewSession).then(res => {
                                    if (res.code == "200") {
                    ...
                    if (symbolAction[`${symbol}`].action == 'L') {
                        ...
                        ExKey.map(v => {
                            if (isClose[v.name] == 1) {
                                BuyClose(symbol, v.amount, v.rToken, v.btSession, v.btNewSession).then(res => {
                                    if (res.code == "200") {
                ...
                if (diffPrice > btPrice1) {
                    if ((sDt[`${symbol}`] == undefined || Date.now() - sDt[`${symbol}`] > sDtTime * 1000) && (sDt2[`${symbol}`] == undefined || sDt2[`${symbol}`] == -1)) {
                        ...
                        if (flag == 'L') {
                            ExKey.map(v => {
                                BuyByQ(symbol, v.amount, v.rToken, v.btSession, v.btNewSession).then(res => {
                                    if (res.code == "200") {
                                        console.log(v.name, '多单开单成功')
                                        isClose[v.name] = 1
                                    } else {
                                        console.log(v.name, '多单开单失败', res)
                                    }
                                }).catch(err => {
                        ...
                        if (flag == 'S') {
                            ExKey.map(v => {
                                SellByQ(symbol, v.amount, v.rToken, v.btSession, v.btNewSession).then(res => {
                                    if (res.code == "200") {
                                        console.log(v.name, '空单开单成功')
                                        isClose[v.name] = 1
                                    } else {
                                        console.log(v.name, '空单开单失败', res)
                                    }
                                }).catch(err => {
