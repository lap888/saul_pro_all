...
const set_ratio = async (symbol) => {
    ...
    if (index > 6) {
        if (ratio_24hr > 0) {
            // 单边上涨，补仓比率不变
            data_json[symbol].config.double_throw_ratio = 0.3 + get_step(symbol) / 6;
            data_json[symbol].config.profit_ratio = 0.3 + get_step(symbol) / 6;
        } else {
            data_json[symbol].config.double_throw_ratio = 0.5 + get_step(symbol) / 6;
            data_json[symbol].config.profit_ratio = 0.3 + get_step(symbol) / 6;
        }
    } else {
        // 系数内震荡行情
...
const calcSlopeMA5 = async (symbol, interval, point, records = '') => {
    ...
    let data = []
    if (records == '') {
        data = await cf.records(symbol, interval, 11)
    } else {
        data = records.slice(-11)
    }
    for (let i = 0; i < data.length; i++) {
        let e = data[i];
        if (i == 0) {
...
const modify_price = (symbol, deal_price, step, market_price) => {
    ...
    modify_json_data(data_json)
    console.log(`${symbol}=>,当前价:${market_price}-|-修改后的补仓价格为:${data_json[symbol].runBet.next_buy_price}-|-修改后的网格价格为:${data_json[symbol].runBet.grid_sell_price}`);
}
