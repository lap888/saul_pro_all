// fmz@edb804d9903c5ef0eb984801fdf414e6
const Ct = "swap";//合约类型
...
// function M5Action(e) {
//     let action = '';
//     let records = e.GetRecords(PERIOD_M1)
//     let ma90 = TA.MA(records, 90)
//     let ema10 = TA.EMA(records, 10)
...
//     } else {
//         //onle 5 min
//         let _records = e.GetRecords(PERIOD_M5)
//         let _ma90 = TA.MA(_records, 90)
//         let _ema10 = TA.EMA(_records, 10)
//         let _ma90price1 = _N(_ma90[_ma90.length - 2], 4)
//         let _ma90price2 = _N(_ma90[_ma90.length - 1], 4)

//         let _ema10price1 = _N(_ema10[_ema10.length - 2], 4)
//         let _ema10price2 = _N(_ema10[_ema10.length - 1], 4)
//         if ((_ma90price1 < _ema10price1 && _ma90price2 > _ema10price2) || (_ma90price1 > _ema10price1 && _ma90price2 > _ema10price2)) {
//             action = 'sell'
//         } else {
//             action = 'buy'
//         }
...
function PrintStatus(pos, OrgAccount) {
    let tbl = {
        type: "table",
        title: "持仓信息",
        cols: ["当前趋势", "币种", "方向", "杠杆", "持仓量", "持仓价", "浮亏", "止盈价", "标记价", "保证金", "补仓次数", "开仓后最低", `激活补仓`, `补仓回调率${_N(RepairReturnRate * 100, 2)}%`],
        rows: []
    }
    for (let i = 0; i < pos.length; i++) {
        let e = pos[i];
        tbl.rows.push([Action, e.Info.symbol, e.Type == 0 ? '多' : '空', e.Info.leverage, e.Amount, e.Price, e.Profit, CoverPrice, e.Info.markPrice, _N(e.Margin, 4), Counter.buy, HistoryMinPrice, `${_N(ReversePrice, 4)}`, `${_N(HistoryReturnRate, 4)}%`])
    }

    let str = `✱牛蛙量化专注马丁类策略的研究，QQ方式：863368681 微信：niuwa10001 #32CD32
    1.可设置回调补仓 eg:浮亏35% 回调5% 相当于浮亏30%补仓，如果没有达到回调比例，则不补仓
    2.可设置锁仓金额，当浮亏达到设置金额时，自动开反向单锁住
    3.可设置资金使用率以及补仓次数，系统会更具你的本金自动算出首仓
    4.策略总来说，盈利稳定，非暴利无限补仓马丁
    5.有资金划转功能，即非复利模式，把合约赚到的U，及时划转到现货账户保住利润
    6.解仓功能正在开发中...
    ...
    tbl2.rows.push([Action, _N(initBalance, 4), _N(Number(OrgAccount.Info.totalMarginBalance), 4), _N(Number(availableBalance), 4), _N(toxh, 4), _N(income, 4), `${_N(incomeRate, 4)}%`, NowBzjRate])
        ...
        try {
            ...
            if (positions.length <= 1 && !IsHaveLock) {
                ...
                if (flag) {
                    ...
                    let isSellFlag = positions.length == 1 && positions[0].Type == 1;
                    if (positions.length == 0 || isSellFlag) {
                        if (positions.length == 1 && positions[0].Type == 1) {
                            if (shortIncome > 0 && shortAmount > 0) {
                                //买入平空
                                exchange.SetDirection("closesell");
                                exchange.Buy(-1, shortAmount);
                            }
                        }
                        //算收益
                        AfterIncomeDo(SymbolCoin);
                        pos_amount = _G(`pos_amount_${SymbolCoin}`)
                        let amount = isSellFlag ? positions[0].Amount : OpAmount.Amount;
                    ...
                    // CalculatorN(pos.Type, minPrice, pos.Price, coin);
                    let sliceN = CalculatorSliceN(pos.Type, ticker.Last, minPrice, pos.Price, coin);
                    NRate = DownGoAddCoinRate(AddCoinRate);
                    let lastAddCoinPricce = _G(`newAddCoinprice`) == null ? pos.Price : _G(`newAddCoinprice`)
                    ReversePrice = _N(lastAddCoinPricce * (1 - (RepairRate * NRate)), pricePrecision);//补仓价
                    NowBzjRate = _N(pos.Amount * ticker.Last / MarginLevel / OrgAccount.Balance, 4);
                    if (ticker.Last < ReversePrice) {
                        //补仓回调 回调买入
                    ...
                    if (shortAmount == 0 && longAmount > 0) {
                        //开空单对冲
                        let pos = Trade(exchange, ORDER_TYPE_SELL, longAmount)
                        if (pos.length == 0) {
                            throw "出师不利, 开空单对冲失败";
                        } else {
                            Log("开空单对冲", "均价:", pos.Price, "数量:", pos.Amount, "#32CD32");
                        }
                    }
                    if (shortAmount > 0 && shortIncome > 0) {
                        //买入平空
                        exchange.SetDirection("closesell");
                        exchange.Buy(-1, shortAmount);
                    }
                ...
                if (!IsHaveLock) {
                    if ((action == 'sell' && shortIncome > 0 && shortAmount > 0 && longAmount == 0) || (shortAmount > 0 && shortIncome > 1 && action == 'buy')) {
                        //买入平空
                        exchange.SetDirection("closesell");
                        exchange.Buy(-1, shortAmount);
                    }
                    let IsHaveAdd = _G('IsHaveAdd') == null ? 0 : _G('IsHaveAdd');
                    //
                    if (shortAmount > 0 && longAmount > 0 && _N(shortIncome, pricePrecision) == _N(income, pricePrecision) && IsHaveAdd == 0) {
                        Log('shortIncome,income', shortIncome, income)
                        let pos1 = Trade(exchange, ORDER_TYPE_SELL, shortAmount)
                        if (pos1.length == 0) {
                            throw "出师不利, 双向加仓失败1";
                        } else {
                            Log("空头加仓，拉近持仓价", "均价:", pos1.Price, "数量:", pos1.Amount, "#32CD32");
                        }
                        let pos2 = Trade(exchange, ORDER_TYPE_BUY, shortAmount)
                        if (pos2.length == 0) {
                            throw "出师不利, 双向加仓失败2";
                        } else {
                            Log("多头加仓，拉近持仓价", "均价:", pos2.Price, "数量:", pos2.Amount, "#32CD32");
                            CancelNoSucessOrder(exchange, 'BUY')
                            let coverPrice = _N(pos2.Price * (1 + StopProfit), pricePrecision);//止盈
                            exchange.SetDirection("closebuy");
                            exchange.Sell(coverPrice, pos2.Amount);
                        }
                        let buyC = Counter.buy = Counter.buy + 1;
                        _G(`${SymbolCoin}_buy`, buyC)
                        _G('IsHaveAdd', 1)
                    }
                }
                Sleep(Interval * 2);
